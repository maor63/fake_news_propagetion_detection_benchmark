...
def get_retweet_path(tweet_propagation_path, propagation_size, tree_delimiter='->'):
...
def extract_user_features(users_df, user_id_field='author_osn_id'):
    ...
    users_features_df['default_profile'] = users_df['default_profile'].astype(np.int8).values
    users_features_df['default_profile_image'] = users_df['default_profile_image'].astype(np.int8).values
...
def main():
    ...
    dataset_sufix = 'fake_news_3500_posts'
    ...
    label_dict = get_label_dict(data_path / 'label.txt')
    ...
    for i, tweet_id in enumerate(label_dict):
        ...
        if os.path.exists(tweet_propagation_path):
            ...
            user_ids_in_propagation = get_retweet_path(tweet_propagation_path, path_len, tree_delimiter)
